...
PID_control::PID_control()
{
	...
	name_pub=n.advertise<std_msgs::String>("name",1);
...
void PID_control::init_variables()
{
	...
	prev_error=0;
//	name_nodo=node_name;
	name_nodo=ros::this_node::getName();
//////////////////////////////////////////////////////////
...
ROS_INFO("parametros:");
		...
		if(n.getParam("rate",rate))
	{
		...
		if(n.getParam( name_nodo +"/kp_vel",kp_vel))
	{
		ROS_INFO_STREAM("Kp_vel from param" << kp_vel);
	}

		if(n.getParam( name_nodo +"/ki_vel",ki_vel))
	{
		ROS_INFO_STREAM("Ki_vel from param" << ki_vel);
	}

		if(n.getParam(name_nodo +"/kd_vel",kd_vel))
	{
		ROS_INFO_STREAM("Kd_vel from param" << kd_vel);
	}	

		if(n.getParam(name_nodo +"/km_vel",km_vel))
	{
		ROS_INFO_STREAM("Km_vel from param" << km_vel);
	}

		if(n.getParam(name_nodo +"range_vel",range_vel))
	{
...
//	{	std::cout <<"argv" << argv[i]<<std::endl;	}
	std::string node=argv[7];
	int size_str=node.length();
	std::string name=node.substr(8,(size_str-8));
	std::cout <<"argv" << name <<std::endl;*/
